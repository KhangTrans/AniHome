...
export const submitAdoptionRequest = async (adoptionData) => {
  try {
    ...
    const payload = {
      ...
      userId: userId,
      fullName: adoptionData.adopterName,
      phone: adoptionData.adopterPhone,
      address: adoptionData.adopterAddress,
      homeType: adoptionData.livingSpace || 'house',
      hasPets: adoptionData.hasOtherPets || false,
      reason: adoptionData.reason,
    };

    console.log('[ADOPTION SERVICE] Current User:', currentUser);
    console.log('[ADOPTION SERVICE] Submitting payload:', payload);
    const response = await axiosInstance.post('/adoptions', payload);

    console.log('[ADOPTION SERVICE] Response:', response.data);
    return {
      success: true,
      data: response.data,
    ...
    console.error('[ADOPTION SERVICE] Error:', error.response?.data || error.message);
    throw {
      success: false,
      message: error.response?.data?.message || 'Failed to submit adoption request',
      error: error
    };
  }
};
