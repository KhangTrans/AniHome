...
import { Card, Input, Button, Tag, Space, Badge, Tooltip, Row, Col, Modal, Spin, Pagination } from 'antd';
import { Plus, Eye, Edit, Trash2, RefreshCw } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import { useToast } from '../../context/ToastContext';
import { getShelterPets, addShelterPet, updatePetStatus, PET_STATUS_OPTIONS } from '../../services/shelter/shelterPetsService';
...
const AnimalManager = () => {
  ...
  }, [shelterID, pagination.page, pagination.pageSize, filter, searchTerm, toast]);
  ...
  const handleAddPet = async (values) => {
    ...
    setFormLoading(true);
    try {
      const result = await addShelterPet(shelterID, values);
            ...
            {PET_STATUS_OPTIONS.map(option => (
              <Button
                ...
                onClick={async () => {
                  modal.destroy();
                  setLoading(true);
                  const result = await updatePetStatus(shelterID, pet.petID, option.value);
              ...
              >
                ...
                {pet.status === option.value && <span style={{ marginLeft: 'auto', color: '#8c8c8c' }}>(hiện tại)</span>}
              </Button>
            ))}
          </Space>
        </div>
    ...
    <Spin spinning={loading}>
      <div>
        ...
        <div style={{ marginBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: '1.75rem', fontWeight: 600 }}>Quản Lý Thú Cưng</h1>
        ...
        <Card style={{ marginBottom: '1.5rem' }}>
          <Space direction="vertical" size="middle" style={{ width: '100%' }}>
            ...
            <Space size="small" wrap>
              <span style={{ color: '#8c8c8c', marginRight: '0.5rem' }}>Lọc theo trạng thái:</span>
              <Button
              ...
              {PET_STATUS_OPTIONS.map(status => (
                <Button
                  ...
                  onClick={() => { setFilter(status.value); setPagination(prev => ({ ...prev, page: 1 })); }}
                  size="middle"
                >
                  {status.label}
                </Button>
          ...
          {pets.map(pet => {
            ...
            return (
              <Col key={pet.petID} xs={24} sm={12} md={8} lg={6}>
                <Badge.Ribbon text={statusDisplay.label} color={statusDisplay.color}>
                  <Card
                    hoverable
                    cover={
                    ...
                    <Card.Meta
                      title={
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <span style={{ fontSize: '1.1rem', fontWeight: 600 }}>{pet.petName}</span>
                      ...
                      description={
                        <div>
                          <Space size="small" wrap style={{ marginBottom: '0.5rem' }}>
        ...
        {pagination.totalCount > pagination.pageSize && (
          <div style={{ display: 'flex', justifyContent: 'center', marginTop: '2rem' }}>
            <Pagination
              ...
              showTotal={(total, range) => `${range[0]}-${range[1]} / ${total} thú cưng`}
            />
          </div>
        )}
        ...
        <PetDetailModal
          ...
          onCancel={() => { setDetailModalVisible(false); setSelectedPet(null); }}
          pet={selectedPet}
        />
      </div>
    </Spin>
